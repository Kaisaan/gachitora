...
inFile = open(f"{sys.argv[1]}", "rb")
outFile = open(f"{sys.argv[1]}.txt", "w", encoding="utf-8")
...
for i in range(lineCount):
    ...
    lineNumber = int.from_bytes(inFile.read(4), byteorder="little")
    unknown2 = int.from_bytes(inFile.read(4), byteorder="little")
    unknown3 = int.from_bytes(inFile.read(4), byteorder="little")

    lineInfo2.append([lineNumber,unknown1, unknown2, unknown3])
...
for i in range(lineCount):
    ...
    line = line.replace("\n", "\\n")
    print(line)
    outFile.write(f"{lineInfo2[i][0]:X}\t{lineInfo2[i][1]:X}\t{lineInfo2[i][2]:X}\t{lineInfo2[i][3]:X}\t{lineInfo1[i][1]:X}\t\t{line}\n")
